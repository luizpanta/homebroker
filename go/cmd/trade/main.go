...
func main() {
	...
	configMap := &ckafka.ConfigMap{ // Conexão do kafka
		"bootstrap.servers": "host.docker.internal:9094", // modificar para variavel de ambiente quando em produção
		"group.id":          "myGroup",
	...
	go func() {
		for msg := range kafkaMsgChan {
			...
			err := json.Unmarshal(msg.Value, &tradeInput) //Unmarshal transforma de json para objeto
			if err != nil {
				panic(err)
			}
